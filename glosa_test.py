...
def rules(veh_ids, veh_ids_E0_0, veh_ids1_E0_1):
    ...
    for x in range(0, list_size0):
        ...
        if current_route == 'route0':
            if (current_lane == 0):
                right_lane_no_vehicles = traci.lane.getLastStepVehicleIDs("gneE0_0")
                left_lane_no_vehicles = len(leaders_left_lane)
                check = len(right_lane_no_vehicles)
                rightlane_pool=0
                while check > 0:
                    if (traci.vehicle.getRouteID(veh_ids_E0_0[check - 1]) == 'route0'):
                        rightlane_pool = rightlane_pool + 1
                        vehicle = veh_ids_E0_0[check - 1]

                    if vehicle==veh_ids[x]:
                        break
                    check = check - 1

                if (rightlane_pool+left_lane_no_vehicles)> TOTAL_VEH_LIMIT :
                    traci.vehicle.setColor(veh_ids[x],(197,66,244))
                else:
